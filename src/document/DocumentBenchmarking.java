...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			 ...
			 */
			//2.
			String sub = getStringFromFile(textfile, numToCheck);
			long StartTime = System.nanoTime();
			for (int i=0; i<trials; i++) {
				BasicDocument basicDoc = new BasicDocument(sub);
				double fS = basicDoc.getFleschScore();
			}
			long EndTime = System.nanoTime();
			double estTime = (EndTime-StartTime)/100000000.0;
			System.out.print(estTime+"\t");
			// below is for efficient
			long eStartTime = System.nanoTime();
			for (int i=0; i<trials; i++) {
				EfficientDocument EffDoc = new EfficientDocument(sub);
				double eS = EffDoc.getFleschScore();
			}
			long eEndTime = System.nanoTime();
			double eestTime = (eEndTime-eStartTime)/100000000.0;
